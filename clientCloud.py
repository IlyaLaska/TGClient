...
import os
import asyncio, subprocess, json, scraper, weatherScraper, votesScraper
...
client = TelegramClient('heroku', os.environ['api_id'], os.environ['api_hash'])
...
async def getWeather(sendTo):
  notification = await client.send_message(sendTo, 'Sending current weather for Kyiv:')
  wea = weatherScraper.scrapeWeather()
  # print(wea)
  if wea:
    weatherWarning = await client.edit_message(notification, '**Current weather for Kyiv:**')
    weather = await client.send_message(sendTo, wea)
    asyncio.create_task(messageKiller(weatherWarning, 14400))
    asyncio.create_task(messageKiller(weather, 14400))
  else:
    msg = await client.send_message(sendTo, 'The station failed to respond, please try again later')
...
async def main():
  ...
  if not (await client.is_user_authorized()):
    ...
    await client.sign_in(os.environ['phone_number'], input('Please input your code:'))
  # string = StringSession.save(client.session)
  # print(string)
  print('Telegram Client running')
  # fict = await client.get_participants('fictonline')
  # print(fict)
  # async with client.action('machendos', 'record-video'):
  #   await asyncio.sleep(20)  # type for 2 seconds
  #   await client.send_message('machendos', 'Surprise!')
...
@client.on(events.NewMessage(pattern='/gb$'))
async def sendVotesCount(e):
  asyncio.create_task(e.message.delete())
  sendTo = 0
  if e.chat_id:
    sendTo = e.chat_id
    ...
    sendTo = e.from_id
  asyncio.create_task(client.send_message(sendTo, votesScraper.getVotes()))


@client.on(events.NewMessage(pattern='/we$'))
async def returnWeather(e):
  asyncio.create_task(e.message.delete())
  sendTo = 0
  if e.chat_id:
    sendTo = e.chat_id
    ...
    sendTo = e.from_id
  await getWeather(sendTo)

async def sendWeatherWithInterval(sendTo):
  global wantWeather
  print(wantWeather)
  while sendTo in wantWeather:
    await getWeather(sendTo)
    await asyncio.sleep(10)
  return

@client.on(events.NewMessage(pattern='/westart'))#decide whether I need to make it work for incoming too - check tt for help
async def startWeatherSender(e):
  sendTo = 0
  if e.chat_id:
    sendTo = e.chat_id
  else:
    sendTo = e.from_id
  global wantWeather
  wantWeather.append(sendTo)
  asyncio.create_task(e.message.delete())
  asyncio.create_task(sendWeatherWithInterval(sendTo))

@client.on(events.NewMessage(pattern='/weend'))#decide whether I need to make it work for incoming too - check tt for help
async def stopWeatherSender(e):
  sendTo = 0
  if e.chat_id:
    sendTo = e.chat_id
  else:
  ...
  msg = ''
  global wantWeather
  if sendTo in wantWeather:
    wantWeather.remove(sendTo)
    msg = await client.send_message(sendTo, 'You successfully unsubscribed from weather updates!')
  else:
    msg = await client.send_message(sendTo, 'You are not subscribed to weather updates! Subscrbie with "/westart"')
  asyncio.create_task(e.message.delete())
  asyncio.create_task(messageKiller(msg, 10))
